        ...
        <div>
          {Object.entries(menuContent.menuCategories).map(([category, items]) => (
            items.slug != 'extra' ? (
              ...
              <h2 className="text-2xl font-light mb-4 text-[#8B7355] border-l-4 border-[#C8B6A6] pl-4">
            ...
            : (
              ...
              <div className="flex flex-wrap gap-x-6 items-center">
                <span className="font-medium text-[#8B7355] mr-2">{getLocalizedName(items.name)}</span>
                {items.menuItems?.map((item, index) => (
                  <span key={index} className="text-sm">
                    {getLocalizedName(item.name)} 
                    <span className="text-[#8B7355]"> +{item.price.price1}₺</span>
                  </span>
                ))}
              </div>
            </div>
